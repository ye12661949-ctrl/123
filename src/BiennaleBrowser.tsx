  ...
  return (
    <section className="biennale-browser" aria-label="双年展年份与单元分类">
      ...
      <div className="biennale-event-tabs">
        {biennaleEvents.map(item => <button key={item.id} className={eventId === item.id ? 'active' : ''} onClick={() => changeEvent(item.id)}>{item.label}</button>)}
      </div>
      <div className="biennale-year-row">
        <b>YEAR / 届次</b>
        <div className="biennale-year-tabs">
          {years.map(item => <button key={item} className={year === item ? 'active' : ''} onClick={() => { setYear(item); onSelect(null); }}>{item}</button>)}
        </div>
        {selected && <button className="biennale-clear" onClick={() => onSelect(null)}>取消本层筛选 ×</button>}
      </div>
      <button className={`biennale-year-all ${selected === yearSelection ? 'active' : ''}`} onClick={() => onSelect(selected === yearSelection ? null : yearSelection)}>
        ...
        <div className="biennale-roster-panel">
          ...
          <div className="biennale-roster-list">
            {venice2026ArtistIds.map((id, index) => {
              ...
              return artist ? <button key={id} onClick={() => onOpenArtist(artist)}><b>{String(index + 1).padStart(3, '0')}</b><span>{artist.name}</span></button> : null;
            })}
          </div>
        </div>
      )}
        ...
        {units.map(unit => {
          ...
          return (
            <article className={`biennale-unit-card ${selected === unit.id ? 'active' : ''}`} key={unit.id}>
              <button className="biennale-unit-select" onClick={() => onSelect(selected === unit.id ? null : unit.id)}>
              ...
              <a href={unit.sourceUrl} target="_blank" rel="noreferrer" aria-label={`查看 ${unit.title} 来源`}><ExternalLink size={12} /></a>
            </article>
          );
        })}
      </div>
