    ...
    <section className="field-directory" id="fields" aria-labelledby="field-directory-title">
      ...
      {domain && <div className="field-directory-content">
        <nav className="field-breadcrumb" aria-label="领域目录路径">
          <button onClick={() => { setDomain(null); resetView(); onDomainSelect(null); }}>领域目录</button>
        ...
        <div className="field-category-tabs" aria-label="领域下的栏目">
          {categories[domain].map(item => <button key={item.id} aria-pressed={category === item.id}
            onClick={() => { setCategory(item.id); resetView(); }}>
        ...
        {domain === 'art' && category === 'biennales'
          ...
          : <>
            ...
            {entry && <section ref={detailRef} className="field-entry-detail" aria-label={entry.name + '栏目详情'}>
              <div className="field-detail-head"><h3>{entry.name}</h3><button onClick={() => setEntryId(null)}>收起栏目</button></div>
              <p>{entry.summary}</p>
              <a href={entry.url} target="_blank" rel="noreferrer">官方目录与资料 ↗</a>
              ...
              {entry.sections && <div className="field-source-sections">
                ...
                {entry.sections.map(section => <article key={section.name}>
                  <a href={section.url} target="_blank" rel="noreferrer">{section.name} ↗</a><p>{section.summary}</p>
                </article>)}
              </div>}
              {entry.id === 'foamtalent' && <div className="prize-roster"><h4>按年份 / 届次</h4><div className="field-category-tabs">{photoEditions.filter(item => item.programId === 'foamtalent').map(item => <button key={item.id} aria-pressed={photoEditionId === item.id} onClick={() => { setPhotoEditionId(item.id); onDirectoryArtistsSelect(item.artistIds); }}>{item.label}</button>)}<button aria-pressed={photoEditionId === 'foamtalent-2016'} onClick={() => { setPhotoEditionId('foamtalent-2016'); onDirectoryArtistsSelect(foamTalent2016ArtistIds); }}>2016</button></div></div>}
              {(entry.category === 'festivals' || entry.category === 'awards') && photoEditions.some(item => item.programId === entry.id) && <div className="prize-roster"><h4>按年份 / 届次</h4><div className="field-category-tabs">{photoEditions.filter(item => item.programId === entry.id).map(item => <button key={item.id} aria-pressed={photoEditionId === item.id} onClick={() => { setPhotoEditionId(item.id); onDirectoryArtistsSelect(item.artistIds); }}>{item.label}</button>)}</div><p className="field-record-note">按官方届次资料建立年度艺术家索引；点击年份后对应摄影师进入下方主 ARTISTS 列表。</p></div>}
