...
export function CompleteArtistArchive({ artist }: { artist: Artist }) {
  const deepArchive = artistArchives[artist.id];
  const archive = deepArchive ?? {
    ...
    projects: artist.projects.map((project, index) => ({
      title: project.title,
      cluster: project.type,
      period: project.year,
      summary: project.facts.join(' '),
      actions: project.facts,
      sourceUrl: artist.sourceUrl,
      images: index === 0 ? artist.images : [],
      relations: [],
    })),
  ...
  return (
    <section className="drawer-section complete-archive-section">
      <div className="section-number">01A</div>
      <div>
        <p className="section-kicker">不是一张代表图，而是逐项目建立作品档案</p>
        <h3>项目档案 / Complete Work Index</h3>
        <div className="archive-progress">
          <div><span>项目目录</span><strong>{archive.projectCoverage}</strong></div>
          <div><span>图像归档</span><strong>{archive.imageCoverage}</strong></div>
        </div>
...
export function CuratorRelations({ curatorId }: { curatorId: string }) {
  const network = curatorNetworks[curatorId];
  if (!network) return null;
  return (
    <div className="curator-network">
      <h4>具体关系网：机构 → 展览 → 艺术家</h4>
      <p className="curator-network-institution">{network.institution}</p>
      <div className="curator-role-row">{network.roles.map(role => <span key={role}>{role}</span>)}</div>
...
export function EcosystemDashboard() {
  return (
    <section className="intelligence-shell ecosystem-shell" id="ecosystem">
      <div className="intelligence-hero">
        <div><p className="eyebrow">ART-WORLD ECOSYSTEM</p><h2>机构 / 奖项生态</h2></div>
        <p>这里不把“MoMA 馆藏”“Turner Prize”“摄影奖”混成同一种成就。我们分别看：机构是什么、怎么形成可见度、奖项如何产生、和哪些策展人 / 展览连接，以及一个艺术家通常怎样从较小节点走向更大的艺术圈。</p>
      </div>
      <div className="ecosystem-subhead"><span>FIELD × TYPE × TIER</span><strong>先知道一个机构处在什么层级</strong></div>
      <InstitutionHierarchy />
      <div className="ecosystem-subhead"><span>DEEP PROFILES</span><strong>再看具体机构怎么运转</strong></div>
      ...
      <div className="ecosystem-subhead"><span>MAJOR PRIZES</span><strong>摄影圈之外也要看</strong></div>
